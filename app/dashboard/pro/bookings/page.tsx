...
import { getProBookings } from './actions';
...
const logger = createLogger({ file: 'app/dashboard/pro/bookings/page.tsx' });
...
export default function CleanerBookingsPage() {
  ...
  const loadBookings = useCallback(async () => {
    if (!user) return;
    try {
      const result = await getProBookings();
      setBookings(result.bookings);
      if (!result.success) {
        setMessage({ type: 'error', text: result.error || 'Failed to load bookings.' });
      }
    } catch (error) {
